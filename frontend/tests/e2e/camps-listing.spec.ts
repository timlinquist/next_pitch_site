...
test('View Details navigates to detail page and Register Now goes to registration form', async ({ page }) => {
  await page.goto('/camps');

  const card = page.locator('.service-card', { hasText: camp.name });
  await card.getByRole('link', { name: 'View Details' }).click();

  await expect(page).toHaveURL(new RegExp(`/camps/${camp.slug}$`));
  await expect(page.getByRole('heading', { name: camp.name })).toBeVisible();
